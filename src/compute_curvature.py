...
def meancurvature(pos, faces):
  ...
  n = pos.shape[0]
  stiff, mass, lumped = laplacebeltrami_FEM_v2(pos, faces)
  ai, av = mass
  torch.sparse.mm(mass, )
  mcf = stiff.spmm(ai, torch.reciprocal(av), n, n, tsparse.spmm(stiff, n, n, pos))
  return mcf.norm(dim=-1, p=2), stiff, mass

def compute_meancurv_diff_l2(perturbed_pos, pos, faces):
  ppos = perturbed_pos
...
def compute_meancurv_diff_abs(perturbed_pos, pos, faces):
  ppos = perturbed_pos
  mcp, _, _  = meancurvature(ppos, faces)
  mc, _, (_, a) = meancurvature(pos, faces)
  diff_curvature = mc-mcp
